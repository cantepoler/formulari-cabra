...
<template>
  <div class="dades-wrapper">
    ...
    <div class="info-box">
      <p>
        Benvingut/da a la XVII edició de La Cabra d'Or!! Aquest formulari és obligatori per tots
        els participants i col·laboradors de la festa i també serveix d'inscripció a l'entitat.
        Moltes gràcies per participar i fer possible un any més La Cabra d'Or!
      </p>
    </div>
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <span v-if="esMenor16" class="field-hint">Com que ets menor de 16 anys, demanem el telèfon del responsable.</span>
      ...
      <div class="form-group full-width" v-if="esMenor16">
        <div class="alert-box">
          ...
          <input type="text" placeholder="Nom i cognom del responsable"
            v-model="personals.responsable" :class="{ 'input-error': err('responsable') }">
    ...
    <div class="consentiments-section">
      ...
      <label class="consent-label" :class="{ 'consent-accepted': consentiments.tractamentDades, 'consent-error': err('tractamentDades') }">
      ...
      <label v-if="esMenor14" class="consent-label"
        ...
        <div class="consent-text">
          ...
          <span class="consent-desc">
            [Text de responsabilitat per a menors de 14 anys — pendent]
          </span>
        </div>
      </label>
      <span v-if="esMenor14 && err('responsabilitatMenors')" class="error-msg consent-error-msg">{{ errors.responsabilitatMenors }}</span>
    </div>
...
<style scoped>
.dades-wrapper { display: flex; flex-direction: column; gap: 28px; }
...
.info-box {
  ...
  border-left: 4px solid #f59e0b; padding: 16px; border-radius: 8px;
}
...
.field-hint { font-size: 0.8rem; color: #92400e; margin-top: 2px; line-height: 1.4; }
...
input[type="text"], input[type="date"], input[type="email"], input[type="tel"] {
  ...
  font-size: 1rem; color: #1f2937; box-sizing: border-box; transition: all 0.2s ease;
  background-color: #f9fafb; font-family: inherit;
}
input:focus { outline: none; border-color: #f59e0b; box-shadow: 0 0 0 4px rgba(245,158,11,0.15); background-color: #fff; }
.input-error { border-color: #ef4444 !important; background-color: #fef2f2 !important; }
.error-msg { font-size: 0.8rem; color: #dc2626; margin-top: 2px; }
...
.alert-box p { margin: 0; color: #b91c1c; font-size: 0.9rem; font-weight: 600; }

.roles-section h3 { margin: 0 0 4px 0; font-size: 1.125rem; color: #374151; font-weight: 600; }
.roles-hint { margin: 0 0 14px 0; font-size: 0.875rem; color: #6b7280; }
.checkbox-group { display: grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap: 10px; }
.checkbox-label {
  display: flex; align-items: center; gap: 12px; cursor: pointer;
  font-size: 0.95rem; color: #4b5563; padding: 12px 14px;
  border: 1px solid #e5e7eb; border-radius: 8px;
  transition: all 0.2s ease; background-color: #fff; font-weight: 500;
}
.checkbox-label:hover { background-color: #fffbeb; border-color: #fde68a; color: #d97706; }
.checkbox-selected { background-color: #fffbeb !important; border-color: #f59e0b !important; color: #b45309 !important; font-weight: 600 !important; }
input[type="checkbox"] { width: 18px; height: 18px; accent-color: #d97706; cursor: pointer; margin: 0; flex-shrink: 0; }
...
.consentiments-section h3 {
  margin: 0 0 4px 0; font-size: 1.05rem; color: #374151; font-weight: 700;
}
...
.consent-label:hover { border-color: #d1d5db; background-color: #fafafa; }
.consent-accepted { border-color: #f59e0b !important; background-color: #fffbeb !important; }
.consent-error { border-color: #fca5a5 !important; background-color: #fef2f2 !important; }

.consent-label input[type="checkbox"] {
  margin-top: 2px; /* alinia amb la primera línia de text */
}

.consent-text { display: flex; flex-direction: column; gap: 4px; }
.consent-title { font-size: 0.9rem; font-weight: 700; color: #374151; }
.consent-accepted .consent-title { color: #b45309; }
.consent-desc { font-size: 0.82rem; color: #6b7280; line-height: 1.5; }
.consent-accepted .consent-desc { color: #92400e; }

.optional-badge {
  display: inline-block; font-size: 0.72rem; font-weight: 600;
  background-color: #e5e7eb; color: #6b7280; padding: 1px 7px;
  border-radius: 999px; margin-left: 6px; vertical-align: middle;
}

.consent-error-msg { margin-top: -4px; }

.error-summary {
  background-color: #fef2f2; border: 1px solid #fca5a5;
  border-left: 4px solid #ef4444; padding: 12px 16px; border-radius: 6px;
}
.error-summary p { margin: 0; color: #b91c1c; font-size: 0.9rem; font-weight: 500; }

.actions { display: flex; justify-content: flex-end; margin-top: 4px; border-top: 1px solid #e5e7eb; padding-top: 24px; }
.btn-primary {
  background-color: #d97706; color: #fff; border: none;
  padding: 14px 32px; font-size: 1rem; font-weight: 600;
  border-radius: 8px; cursor: pointer; transition: all 0.2s ease;
  font-family: inherit; box-shadow: 0 4px 6px rgba(217,119,6,0.2);
}
.btn-primary:hover { background-color: #b45309; box-shadow: 0 6px 12px rgba(217,119,6,0.3); transform: translateY(-1px); }
