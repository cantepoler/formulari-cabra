...
export default async function handler(req, res) {
    ...
    try {
        ...
        const [errorsSheets, errorsMail] = await Promise.all([
            sincronitzaSheets(filaInserida),
            enviaMailsInscripcio(inscripcio),
        ]);
        if (errorsSheets.length) console.error('Errors sincronitzant Sheets:', errorsSheets);
        if (errorsMail.length) console.error('Errors enviant correus:', errorsMail);
